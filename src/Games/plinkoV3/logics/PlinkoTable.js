...
class Tooltip extends Container {  // Changed from Sprite to Container
  static width = 560;
  static height = 144;
  static toolLabelStyle = {
    fontSize: 28,
    fill: 0x303D5D,
    fontWeight: '700',
    fontFamily: 'Montserrat',
  };
  static toolTextStyle = {
    fontSize: 28,
    fill: 0x303D5D,
    fontWeight: '700',
    fontFamily: 'Montserrat',
  ...
  constructor() {
    ...
    // Create a background sprite instead of setting texture directly
    this.background = new Sprite(PlinkoResource.LOADED['tooltip']);
    this.background.width = Tooltip.width;
    this.background.height = Tooltip.height;
    this.addChild(this.background);
    
    this.visible = false;
    this.pivot.x = Tooltip.width / 2;
    this.pivot.y = Tooltip.height / 2;
  ...
  drawArrow() {
    ...
    this.arrow.x = Tooltip.width / 2;
    this.arrow.anchor.x = 0.5;
    this.arrow.y = Tooltip.height;
  ...
  drawLabel() {
    ...
    label2.x = Tooltip.width / 2 + this.padding;
  ...
  drawChance() {
    this.chance = new Text({
      text: '34.05%',
      style: Tooltip.toolTextStyle
    });
    this.chance.x = (Tooltip.width / 2) + this.padding + 10;
...
export default class PlinkoTable extends Container {
  ...
  drawPinsContainer() {
    ...
    let pinSpacing = (PlinkoTable.height - this.pinsDistance) / (this.plinko.rows + 1);
    ...
    // Create target containers at the bottom
    ...
    // Safely access payout and chance values
    const getPayoutValue = (i) => {
      try {
        // Check if payout structure exists and has the required properties
        if (this.plinko.payout && 
            this.plinko.payout[this.plinko.risk] && 
            this.plinko.payout[this.plinko.risk][this.plinko.rows]) {
          return this.plinko.payout[this.plinko.risk][this.plinko.rows][i] || 0;
        }
        // Fallback values based on position
        return i === 0 || i === this.plinko.rows ? 5 : 1;
      } catch (error) {
        console.error("Error accessing payout value:", error);
        // Fallback values based on position
        return i === 0 || i === this.plinko.rows ? 5 : 1;
      }
    };
    
    const getChanceValue = (i) => {
      try {
        // Check if chance structure exists and has the required properties
        if (this.plinko.chance && this.plinko.chance[this.plinko.rows]) {
          return this.plinko.chance[this.plinko.rows][i] || 0.01;
        }
        // Fallback to a simple probability calculation
        return 1 / (this.plinko.rows + 1);
      } catch (error) {
        console.error("Error accessing chance value:", error);
        // Fallback to a simple probability calculation
        return 1 / (this.plinko.rows + 1);
      }
    };
    ...
    for (let i = 0; i < this.plinko.rows + 1; i++) {
      let payout = getPayoutValue(i);
      let chance = getChanceValue(i);
      ...
      const handlePointerOver = () => {
        let amount = "0";
        try {
          if (this.game && this.game.amount && typeof this.game.amount.mul === 'function') {
            amount = this.game.amount.mul(payout).toString().slice(0, 10);
          } else {
            amount = (payout * (this.game?.amount || 1)).toFixed(2);
          }
        } catch (error) {
        ...
        this.tooltip.show(
          pin.x,
          pin.y,
          `${amount} ${this.game?.currencyName || 'USD'}`,
          chance
        );
      };
    ...
    // Apply color tint to targets
    for (let i = 0; i < this.targets.length; i++) {
      let tintFactor = (2 * Math.abs(i - (this.targets.length - 1) / 2)) / this.targets.length;
      this.targets[i].sprite.tint = PlinkoResource.interpolateColor ? 
        PlinkoResource.interpolateColor(tintFactor) : 
        0xFFFFFF; // Default white if interpolateColor is not defined
    }
  }
