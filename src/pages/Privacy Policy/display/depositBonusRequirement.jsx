import React from 'react'

export default function depositBonusRequirement() {
  return (
    <div style={{
      maxWidth: '900px',
      margin: '40px auto',
      padding: '30px',
      // backgroundColor: '#ffffff',
      borderRadius: '10px',
      fontFamily: 'Segoe UI, sans-serif',
      fontSize: '16px',
      // color: '#333',
      lineHeight: '1.7',
      boxShadow: '0 4px 12px rgba(0, 0, 0, 0.1)',
      overflowY: 'scroll',
      maxHeight: '90vh'
    }}>
      <h2 style={{color: '#fff', paddingBottom: '5px', fontWeight: "bold",  fontSize: '28px' }}>
        Deposit Bonus Requirement
      </h2>
      <p>
        A bonus rollover defines the multiplier in which the sum of your deposit and bonus will need to be turned over before you are able to make a withdrawal. The majority of our games have between a 3.5% to 4.5% house edge, so we’ve set the wager requirement at a 4% edge.
      </p>
      <p>
        This means that if you play a game with a house edge of 1%, it will complete at a 4x slower rate. Meaning, the requirement accumulates at $0.25 for every $1 wagered. The higher the house edge, the less wager required to complete the requirement.
      </p>
      <p>
        While having an active wager requirement, your chosen currency will be locked until meeting the requirement set in the specific currency, or alternatively using the entire balance (in the relative bonus currency) and also having no active bets.
      </p>
      <p>
        If a Deposit Bonus has been enabled, you can check the progress in the VIP modal.
      </p>
      <p>
        While having an active wager requirement, you cannot wager on certain games. These are listed below:
      </p>

      <div style={{
        // backgroundColor: '#f9f9f9',
        padding: '20px',
        borderRadius: '8px',
        marginTop: '20px'
      }}>
        <h3 style={{color: '#fff', paddingBottom: '5px', fontWeight: "bold" , fontSize: '18px'}}>3 Oaks Gaming</h3>
        <ul>
          <li>All games</li>
        </ul>

        <h3 style={{ marginTop: '20px', marginBottom: '10px', color: '#fff', fontWeight: "bold", fontSize: '18px' }}>AvatarUX</h3>
        <ul>
          <li>7 Lucky Gods</li>
          <li>King of Samba</li>
          <li>Majestic Meow</li>
          <li>Tik Talkers</li>
          <li>Toad's Bounty</li>
        </ul>

        <h3 style={{ marginTop: '20px', marginBottom: '10px', color: '#fff', fontWeight: "bold", fontSize:'18px' }}>Belatra</h3>
        <ul>
          <li>7 Days The Spanish Armada</li>
          <li>Full Moon Magic</li>
          <li>Merry Hog</li>
          <li>Richy Hog</li>
          <li>Zombie Town</li>
        </ul>

        <h3 style={{ marginTop: '20px', marginBottom: '10px', color: '#fff', fontWeight: "bold",  fontSize: '18px' }}>BGaming</h3>
        <ul>
          <li>Adventures</li>
          <li>Blackbeard’s Bounty</li>
          <li>Candy Monsta</li>
          <li>Catch the Gold Hold And Win</li>
          <li>Cat's Soup</li>
          <li>God of Wealth Hold And Win</li>
          <li>Hit The Route</li>
          <li>Infinity Pull</li>
          <li>Johnny Cash</li>
          <li>Maneki 88 Gold</li>
          <li>Mice & Magic Wonder Spin</li>
          <li>Road 2 Riches</li>
          <li>Stake Million</li>
          <li>Tile Master</li>
          <li>WBC Ring of Riches</li>
          <li>Wild Heart</li>
          <li>Wild West Trueways</li>
        </ul>

        <h3 style={{ marginTop: '20px', marginBottom: '10px', color: '#fff', fontWeight: "bold", fontSize: '18px' }}>Big Time Gaming</h3>
        <ul>
          <li>Golden Goose Megaways</li>
          <li>Holy Diver Megaways</li>
          <li>Lil Devil</li>
          <li>Royal Mint</li>
        </ul>

        <h3 style={{ marginTop: '20px', marginBottom: '10px', color: '#fff', fontWeight: "bold", fontSize: '18px' }}>Game Titles</h3>
        <ul>
        <li>Blueprint</li>
        <li>Bankin' More Bacon</li>
        <li>Big Catch Even Bigger Bass</li>
        <li>Bounty Hunter: Unchained</li>
        <li>Blazing Cash</li>
        <li>Cash Strike</li>
        <li>Cash Strike Hot Stepper</li>
        <li>Cash Strike Hot Stepper Megaways</li>
        <li>Cash Strike Power Up</li>
        <li>Cash Strike Win Boost</li>
        <li>Cop the Lot Megaways Power Play</li>
        <li>Fat Stacks Miami Cash</li>
        <li>Funky Buddha</li>
        <li>Gold Strike</li>
        <li>King Kong Cash</li>
        <li>King Kong Cash DJ</li>
        <li>King Kong Cash Even Bigger Bananas Christmas</li>
        <li>King Kong Cash Even Bigger Bananas Megaways</li>
        <li>King Kong Cash Even Bigger Bananas 2</li>
        <li>King Kong Cash Even Bigger Bananas 4</li>
        <li>King Kong Cash Full House</li>
        <li>Kong 3 Even Bigger Bonus</li>
        <li>Madame of Mystic Manor</li>
        <li>Majestic Fury Megaways</li>
        <li>Majestic Fury Megaways Unleashed</li>
        <li>Majestic Fury Win Stepper</li>
        <li>Maradona El Diez</li>
        <li>Midas King of Gold</li>
        <li>Plenty O' Fish</li>
        <li>Rise of Atlantis</li>
        <li>Rise of Atlantis 2</li>
        <li>Saint Nicked 2</li>
        <li>The G.O.A.T</li>
        </ul>

        <h3 style={{ marginTop: '20px', marginBottom: '10px', color: '#fff', fontWeight: "bold", fontSize: '18px' }}>Bombay Live</h3>
        <ul>
          <li>Royal Riches Roulette</li>
        </ul>
      

  <h3 style={{ marginTop: '20px', marginBottom: '10px', color: '#fff', fontWeight: "bold",  fontSize: '18px'}}>Booming Games</h3>
        <ul>
          <li>Bang Bang Reloaded</li>
          <li>Billy Bob Boom</li>
          <li>Butterfly Charms</li>
          <li>Buffalo Hold and Win Extreme</li>
          <li>Buffalo Hold and Win Extreme 10,000</li>
          <li>Fortune & Finery</li>
          <li>Hot Charge</li>
          <li>Money Inc</li>
          <li>Red Hot Volcano</li>
          <li>Treasure Vault</li>
          <li>Wombaroo</li>
        </ul>
      </div>


        <h3 style={{ marginTop: '20px', marginBottom: '10px', color: '#fff', fontWeight: "bold", fontSize: '18px' }}>ELK Studios</h3>
        <ul>
          <li>Birthday!</li>
          <li>Bloopers</li>
          <li>Ecuador Gold</li>
          <li>Electric Sam</li>
          <li>Kaiju</li>
          <li>Lake's Five</li>
          <li>Miss Wildfire</li>
          <li>Poltava</li>
          <li>Sam on the Beach</li>
        </ul>
      


        <h3 style={{ marginTop: '20px', marginBottom: '10px', color: '#fff', fontWeight: "bold", fontSize: '18px' }}>Endorphina</h3>
        <ul>
          <li>2025 Hit Slot</li>
          <li>Amazons Riches</li>
          <li>Crown Coins</li>
          <li>Football Superstar</li>
          <li>Fortune Snake</li>
          <li>Golden Brew</li>
          <li>Golden Ox</li>
          <li>Jade Coins</li>
          <li>Jannisaries</li>
          <li>Joker Ra: Sunrise</li>
          <li>Juicy Lips</li>
          <li>Lord of the Seas</li>
          <li>Lucky Streak 1000</li>
          <li>Lucky Streak X</li>
          <li>Moon Tiger</li>
          <li>Ninja</li>
          <li>Oriental Dragon</li>
          <li>Prestige Crown</li>
          <li>Sticky Lips</li>
          <li>Jolly Santa</li>
        </ul>
      


        <h3 style={{ marginTop: '20px', marginBottom: '10px', color: '#fff', fontWeight: "bold", fontSize: '18px' }}>Evolution</h3>
        <ul>
          <li>Auto-Roulette</li>
          <li>Bac Bo</li>
          <li>First Person Lightning Blackjack</li>
          <li>Lightning Blackjack</li>
          <li>Stock Market</li>
          <li>VIP Roulette</li>
          <li>Crazy Pachinko</li>
        </ul>
     


        <h3 style={{ marginTop: '20px', marginBottom: '10px', color: '#fff', fontWeight: "bold", fontSize: '18px' }}>Fantasma Games</h3>
        <ul>
          <li>1001 Nights of Riches</li>
          <li>24 Star Dream</li>
          <li>Aphrodite's Fortune</li>
          <li>Circle of Sylvan</li>
          <li>Fortune Llama</li>
          <li>Fortune Llama Hyper Heist</li>
          <li>Gold Pigger</li>
          <li>Gold Pigger 2 Royal Snouts</li>
          <li>Hades River of Souls</li>
          <li>Miss Rainbow</li>
          <li>Prometheus: Titan Of Fire</li>
          <li>Santa's Golden Christmas</li>
          <li>Serpent Gold Coins</li>
          <li>Seven and Gems</li>
          <li>Shadow Summoner Egypt</li>
          <li>Shadow Summoner Elementals</li>
          <li>Vulcan's Gold</li>
        </ul>
     


        <h3 style={{ marginTop: '20px', marginBottom: '10px', color: '#fff', fontWeight: "bold", fontSize: '18px' }}>Fat Panda</h3>
        <ul>
          <li>Lucky Monkey</li>
          <li>Lucky Mouse</li>
          <li>Lucky Phoenix</li>
          <li>Lucky Tiger</li>
          <li>Lucky Tiger 1000</li>
        </ul>
   


        <h3 style={{ marginTop: '20px', marginBottom: '10px', color: '#fff', fontWeight: "bold", fontSize: '18px' }}>GameArt</h3>
        <ul>
          <li>Angry Dragons</li>
          <li>Dead Mouse Adventures</li>
          <li>Fate’s Fury</li>
          <li>Great Buffalo Megaways</li>
          <li>Lucky Loser</li>
          <li>Midgard's Fortune Megaways</li>
          <li>Pirate’s Pearl Megaways</li>
          <li>Safari Gems</li>
          <li>Santa's Downfall</li>
          <li>Totem Token</li>
          <li>Wild Marmalade</li>
        </ul>
      
        <ul>
          <li>11 Enchanting Relics</li>
          <li>1st Of The Irish</li>
          <li>3 Lucky Rainbows</li>
          <li>9 Skulls of Gold</li>
          <li>Africa X UP</li>
          <li>African Legends</li>
          <li>Age of Conquest</li>
          <li>Agnes Mission: Wild Lab</li>
          <li>Akiva: Claws of Power</li>
          <li>Amazing Link Apollo</li>
          <li>Amazing Link Bounty</li>
          <li>Amazing Link Hades</li>
          <li>Amazon Kingdom</li>
          <li>Amazon: Lost Gold</li>
          <li>Amber Sterling's Mystic Shrine</li>
          <li>Ark of Ra</li>
          <li>Augustus</li>
          <li>Avalon</li>
          <li>Bling Bling Penguin</li>
          <li>Bolt X Up</li>
          <li>Book of Oz</li>
          <li>Buffalo on Fire!</li>
          <li>Bust The Mansion</li>
          <li>Captain Wild</li>
          <li>Chronicles of Olympus X UP</li>
          <li>Crazy Digginz: It's All Mine!</li>
          <li>Diamond King Gold</li>
          <li>Diamond King Jackpots</li>
          <li>Diamond Sands</li>
          <li>Dr. Wildshock: Mad Loot Lab</li>
          <li>Fiona’s Christmas Fortune</li>
          <li>Fire and Roses Joker</li>
          <li>Fish 'Em Up</li>
          <li>Fishin' Christmas Pots Of Gold</li>
          <li>Fishin' Pots of Gold: Gold Blitz</li>
          <li>Fishing Floats of Cash</li>
          <li>Gallo Gold Bruno's Megaways</li>
          <li>God of Fire</li>
          <li>Gustav Minebuster</li>
          <li>Immortal Romance</li>
          <li>Lucky Clucks</li>
          <li>Lucky Leprechaun Clusters</li>
          <li>Lucky Twins Link&Win</li>
          <li>Mustang Riches</li>
          <li>Oink Farm</li>
          <li>Outlaw Saloon</li>
          <li>Peaky Pigs</li>
          <li>Pile 'Em Up</li>
          <li>Piles of Presents</li>
          <li>Pirates Quest</li>
          <li>Retro Reels - Extreme Heat</li>
          <li>Scarab Kingdom</li>
          <li>Squealin' Riches</li>
          <li>Super 30 Stars</li>
          <li>The Finer Reels of Life</li>
          <li>Thunderstruck</li>
          <li>Thunderstruck II</li>
          <li>Thunderstruck Stormchaser</li>
          <li>Ticket to Riches</li>
          <li>Treasures Of Kilauea</li>
          <li>Triple Irish</li>
          <li>Trojan Kingdom</li>
          <li>Unusual Suspects</li>
          <li>Viking Queen</li>
          <li>Vikings Journey</li>
          <li>Wild Catch</li>
          <li>Wild Link Frenzy</li>
          <li>Wild Orient</li>
          <li>Witch Feature</li>
        </ul>
     


        <h3 style={{color: '#fff', paddingBottom: '5px', fontWeight: "bold", fontSize: '18px' }}>Hacksaw Gaming</h3>
        <ul>
          <li>Stick'Em</li>
        </ul>
      


        <h3 style={{color: '#fff', paddingBottom: '5px', fontWeight: "bold", fontSize: '18px' }}>Jade Rabbit</h3>
        <ul>
          <li>Ace Race</li>
          <li>Angler's Paradise</li>
          <li>Steam Spin</li>
          <li>Red Hot Reels</li>
        </ul>
      


        <h3 style={{color: '#fff', paddingBottom: '5px', fontWeight: "bold", fontSize: '18px' }}>Just Slots</h3>
        <ul>
          <li>Shogun SkyLord</li>
        </ul>


        <h3 style={{color: '#fff', paddingBottom: '5px', fontWeight: "bold", fontSize: '18px' }}>NetEnt</h3>
        <ul>
          <li>Babylon Riches</li>
          <li>Buckshot Wilds</li>
          <li>Codex of Fortune</li>
          <li>Crab Trap</li>
          <li>Cursed Treasure</li>
          <li>Dead or Alive</li>
          <li>Gladiator Clash</li>
          <li>Golden Egg Invaders</li>
          <li>Dragons Playground</li>
          <li>Mermaids</li>
          <li>Let It Burn</li>
          <li>Lost Relics 2</li>
          <li>Jack Hammer 3</li>
          <li>Jingle Bells Bonanza</li>
          <li>Mummy Megaways</li>
          <li>Rabid Randy</li>
          <li>Rainforest Gold</li>
          <li>Rome: The Golden Age</li>
          <li>Pirate's Party</li>
          <li>Serengeti Kings</li>
          <li>Street Fighter 2</li>
          <li>The Wish Master</li>
          <li>The Wish Master Megaways</li>
          <li>Thrill To Grill</li>
          <li>Ticket To Wild</li>
        </ul>
      


        <h3 style={{color: '#fff', paddingBottom: '5px', fontWeight: "bold", fontSize: '18px' }}>Novomatic</h3>
        <ul>
          <li>8 Bullets Hold & Win</li>
          <li>Christmas Boom</li>
          <li>Jackass Gold Hold & Win</li>
          <li>Charming Lady's Boom</li>
          <li>Vault Lock: Crack it Big</li>
        </ul>
      


        <h2 style={{color: '#fff', paddingBottom: '5px', fontWeight: "bold", fontSize: '18px' }}>Octoplay</h2>
        <ul>
          <li>777 Hot Reels: Supercharged</li>
          <li>Break the Piggy Bank</li>
          <li>Buffalo Blaze: Wild Wealth</li>
          <li>Christmas Link Express: Hold and Win</li>
          <li>Flame & Fortune: Hold & Win</li>
          <li>Juicy Jewels Supercharged</li>
          <li>Piggy Coin: Hold and Win</li>
          <li>Pip & Pippa's Pots of Plenty!</li>
          <li>Rooster's Gold: Hold and Win</li>
          <li>Super Cash Boost: Hold and Win</li>
          <li>The Big Bark Golden Jackpot</li>
          <li>Twin Cash: Hold and Win</li>
        </ul>
      

      <h2 style={{color: '#fff', paddingBottom: '5px', fontWeight: "bold", fontSize: '18px' }}>OneTouch</h2>
      <ul>
        <li>Flexing Dragons</li>
        <li>Gifts Of Horus</li>
        <li>Reel Fruitz Frenzy</li>
        <li>Queen of Glory Legacy</li>
        <li>Steam Vault</li>
        <li>Tiki Terror</li>
        <li>Wacky Wildlife</li>
        <li>Wealthy Sharks</li>
        <li>Wild Voodoo</li>
      </ul>


      <h2 style={{color: '#fff', paddingBottom: '5px', fontWeight: "bold", fontSize: '18px' }}>Peter & Sons</h2>
      <ul>
        <li>Animafia</li>
        <li>Blood Club</li>
        <li>Johnan Legendarian</li>
        <li>Peter Hunter</li>
        <li>Robin - Sherwood Marauders</li>
        <li>Robin - Nottingham Raiders</li>
        <li>Sands of Destiny</li>
        <li>Steamworks - Gears of Fortune</li>
        <li>The Legend of Musashi</li>
        <li>Thunder Hawk</li>
        <li>Zombies - At the door</li>
      </ul>


      <h2 style={{color: '#fff', paddingBottom: '5px', fontWeight: "bold", fontSize: '18px' }}>PG Soft</h2>
      <ul>
        <li>Dragon Hatch</li>
        <li>Dragon Hatch 2</li>
        <li>Fortune Dragon</li>
        <li>Fortune Rabbit</li>
        <li>Fortune Tiger</li>
        <li>Fortune Mouse</li>
        <li>Fortune Ox</li>
        <li>Futebol Fever</li>
      </ul>


      <h2 style={{color: '#fff', paddingBottom: '5px', fontWeight: "bold", fontSize: '18px' }}>PlaynGo</h2>
      <ul>
        <li>1001 Mystery Genie Fortunes</li>
        <li>3 Clown Monty 2</li>
        <li>15 Crystal Roses: A Tale of Love</li>
        <li>Baker's Treat</li>
        <li>Bucking Rider</li>
        <li>Boat Bonanza Rider</li>
        <li>Cash Surge</li>
        <li>Cat Wilde and The Incan Quest</li>
        <li>Chambers of Ancients</li>
        <li>City of Sound</li>
        <li>Crabby's Gold</li>
        <li>Eye of the Kraken</li>
        <li>Fulong 88</li>
        <li>GEMiX</li>
        <li>Gerard's Gambit</li>
        <li>Golden Legend</li>
        <li>Happy Halloween</li>
        <li>Helloween</li>
        <li>Hugo 2</li>
        <li>Legion Gold Victory</li>
        <li>Love is in the Fair</li>
        <li>Loot & Labyrinths</li>
        <li>Mahjong 88</li>
        <li>Minor Donkey Rider</li>
        <li>Moon Princess</li>
        <li>Moon Princess 100</li>
        <li>Moon Princess Power of Love</li>
        <li>Moon Princess Stargazing</li>
        <li>Multifruit 81</li>
        <li>Pearls of India</li>
        <li>Piggy Blitz Disco Gold</li>
        <li>Pimped</li>
        <li>Rage to Riches</li>
        <li>Rich Wilde and the Pearls of Vishnu</li>
        <li>Royal Masquerade</li>
        <li>Sea Hunter</li>
        <li>Tower Quest</li>
        <li>Viking Runecraft</li>
      </ul>


      <h2 style={{color: '#fff', paddingBottom: '5px', fontWeight: "bold", fontSize: '18px' }}>Playson</h2>
      <ul>
        <li>All games</li>
      </ul>


      <h2 style={{color: '#fff', paddingBottom: '5px', fontWeight: "bold", fontSize: '18px' }}>PopiPlay</h2>
      <ul>
        <li>Guns and Dragons</li>
        <li>Mythos</li>
        <li>Slotham City</li>
        <li>Wild Wild Bank</li>
      </ul>

      <h2 style={{ marginTop: '0', marginBottom: '10px', color: '#fff', fontWeight: "bold", fontSize: '18px' }}>Pragmatic Play</h2>
      <ul>
        <li>3 Kingdoms - Battle of Red Cliffs</li>
        <li>5 Lions Gold</li>
        <li>7 Clovers of Fortune</li>
        <li>Asgard</li>
        <li>Candy Jar Clusters</li>
        <li>Cash Elevator</li>
        <li>Chests of Cai Shen</li>
        <li>Ding Dong Christmas Bells</li>
        <li>Dragon Kingdom</li>
        <li>Dragon King Hot Pots</li>
        <li>Gem Trio</li>
        <li>Gold Train</li>
        <li>Jade Butterfly</li>
        <li>Jungle Gorilla</li>
        <li>Lobster Bob's Sea Food and Win It</li>
        <li>Lucky's Wild Pub</li>
        <li>Money Stacks Megaways</li>
        <li>Mummy's Jewels</li>
        <li>Pinup Girls</li>
        <li>Sleeping Dragon</li>
        <li>Snakes and Ladders Megadice</li>
        <li>Spaceman</li>
        <li>The Champions</li>
        <li>Tiny Toads</li>
        <li>Transylvania Mania</li>
        <li>Triple Pot Gold</li>
        <li>Vampy Party</li>
        <li>Wheel O'Gold</li>
        <li>Wild Depths</li>
        <li>Witch Heart Megaways</li>
      </ul>

      <h2 style={{ marginTop: '0', marginBottom: '10px', color: '#fff', fontWeight: "bold", fontSize: '18px' }}>Print Studios</h2>
      <ul>
        <li>Big Stack Lumberjack</li>
        <li>Honey Hunters</li>
      </ul>

      <h2 style={{ marginTop: '0', marginBottom: '10px', color: '#fff', fontWeight: "bold", fontSize: '18px' }}>Push Gaming</h2>
      <ul>
        <li>Big Bite Push Ways</li>
        <li>Blaze of Ra</li>
        <li>Bison Battle</li>
        <li>DJ Fox</li>
        <li>Fish 'N' Nudge</li>
        <li>Fish 'N' Nudge Big Catch</li>
        <li>Hearts Highway</li>
        <li>Jammin Jars 2</li>
        <li>Mad Cars</li>
        <li>Power Vault</li>
        <li>Razor Shark</li>
        <li>Razor Returns</li>
        <li>Samurai's Katana</li>
        <li>Shamrock Saints</li>
        <li>Tiki Tumble</li>
        <li>Wild Swarm</li>
        <li>Wild Swarm 2</li>
        <li>Wild Swarm Triple Hive</li>
      </ul>

      <h2 style={{ marginTop: '0', marginBottom: '10px', color: '#fff', fontWeight: "bold", fontSize: '18px' }}>Quickspin</h2>
      <ul>
        <li>Brawlers Bar Cash Collect</li>
        <li>Crown of Valor</li>
        <li>Skulls Up!</li>
        <li>Shields of Lambda</li>
      </ul>

      <h2 style={{ marginTop: '0', marginBottom: '10px', color: '#fff', fontWeight: "bold", fontSize: '18px' }}>Red Rake Gaming</h2>
      <ul>
        <li>Boomerang Jack's Lost Mines</li>
        <li>Captain Wild</li>
        <li>Guardians of Luxor 3</li>
        <li>Gustav Minebuster</li>
        <li>Midas Wins</li>
        <li>Million 777 Bells</li>
        <li>Million 777 Wheel</li>
        <li>Million Christmas</li>
        <li>Million Christmas 2</li>
        <li>Million Zeus</li>
        <li>Million Zeus 2</li>
        <li>Ramses Legacy</li>
        <li>Super 30 Stars</li>
        <li>Super 60 Stars</li>
        <li>Triple Irish</li>
        <li>Vikings Journey</li>
      </ul>

      <h2 style={{color: '#fff',  paddingBottom: '5px', fontWeight: "bold", fontSize: '18px' }}>Red Tiger</h2>
      <ul>
        <li>10,001 Nights Megaways</li>
        <li>Bounty Raid 2</li>
        <li>Buffalo Collector</li>
        <li>Dear Santa</li>
        <li>Diamond Blitz 2</li>
        <li>Diamond Doggies</li>
        <li>Dont Hit Plz</li>
        <li>Dragon's Clusterbuster</li>
        <li>Dragon's Fire Infinireels</li>
        <li>Dragon's Luck Megaways</li>
        <li>Dragon's Mirror</li>
        <li>Dynamite Riches</li>
        <li>Dynamite Riches Megaways</li>
        <li>Egypt Clusterbuster</li>
        <li>Fa Fa Babies 2</li>
        <li>Get the Gold Infinireels</li>
        <li>Gladiator Ways</li>
        <li>Gods of Troy Megaways</li>
        <li>Gold Mine Mistress</li>
        <li>Gonzo's Quest Megaways</li>
        <li>Jackrabbit Jackpots</li>
        <li>London Tube</li>
        <li>Miami Rise</li>
        <li>Midnight Thirst</li>
        <li>Monster Unchained</li>
        <li>Play with the Devil</li>
        <li>Primate King Megaways</li>
        <li>Reptizillions Power Reels</li>
        <li>Rise of Cleopatra</li>
        <li>Roman Emperors</li>
        <li>Sea Boat Adventure Megaways</li>
        <li>Serpent King</li>
        <li>Sword Stomp</li>
        <li>Siren's Riches</li>
        <li>Spin Town</li>
        <li>Spooky Carnival</li>
        <li>The Wild Kiss</li>
        <li>Turtle Paradise</li>
        <li>Ultra Rich</li>
        <li>Zaida's Fortune</li>
        <li>Zeus Lightning Power Reels</li>
        <li>Zillard King</li>
        <li>Zillard King Megaways</li>
      </ul>



<h2 style={{color: '#fff',  paddingBottom: '5px', fontWeight: "bold", fontSize: '18px' }}>Relax Gaming</h2>
      <ul>
        <li>10,000 Years Before Cash</li>
        <li>5 Monsters</li>
        <li>Book of 99</li>
        <li>Christmas Santa</li>
        <li>Deep Descent</li>
        <li>Feather Fury</li>
        <li>Frequent Flyer</li>
        <li>Gold Rush Frenzy Megaways</li>
        <li>Great Pigsby Megaways</li>
        <li>Hellcatraz</li>
        <li>Money Sleigh</li>
        <li>Phoenix Up Cash</li>
        <li>Temple Rush</li>
        <li>The Wild Drifter</li>
        <li>Titan Strike</li>
        <li>Treasure Pirates</li>
        <li>Wild Donuts</li>
        <li>Wild Buccaneers Triple Treasure</li>
      </ul>


      <h2 style={{color: '#fff',  paddingBottom: '5px', fontWeight: "bold", fontSize: '18px' }}>Slotmill</h2>
      <ul>
        <li>Cash Pandas</li>
        <li>Sands of Eternity</li>
      </ul>


      <h2 style={{color: '#fff',  paddingBottom: '5px', fontWeight: "bold", fontSize: '18px' }}>Spinomenal</h2>
      <ul>
        <li>1 Reel Baba Yaga</li>
        <li>1 Reel Beauty</li>
        <li>1 Reel Buffalo</li>
        <li>1 Reel - Caribbean Treasure</li>
        <li>1 Reel: Cash Vault</li>
        <li>1 Reel Demi Gods II</li>
        <li>1 Reel Demi Gods III</li>
        <li>1 Reel Demi Gods IV</li>
        <li>1 Reel Egypt</li>
        <li>1 Reel Elves</li>
        <li>1 Reel Fruits</li>
        <li>1 Reel Golden Piggy</li>
        <li>1 Reel Halloween</li>
        <li>1 Reel Hawaiian Bliss</li>
        <li>1 Reel Joker</li>
        <li>1 Reel Majestic King</li>
        <li>1 Reel Mammoth</li>
        <li>1 Reel Monkey</li>
        <li>1 Reel Panther</li>
        <li>1 Reel Patrick</li>
        <li>1 Reel Reef</li>
        <li>1 Reel Shamrock</li>
        <li>1 Reel Thanksgiving</li>
        <li>1 Reel Wolf Fang</li>
        <li>1 Reel Xmas</li>
        <li>Aztec Spell</li>
        <li>Aztec Spell - 10 Lines</li>
        <li>Beer Collection - 20 Lines</li>
        <li>Beer Collection - 30 Lines</li>
        <li>Book of Alice</li>
        <li>Book Of Demi Gods III</li>
        <li>Book Of Rebirth</li>
        <li>Book of The Divine</li>
        <li>Colossus Fruits</li>
        <li>Comic Craze</li>
        <li>Dwarfs Gone Wild</li>
        <li>Demi Gods III</li>
        <li>Egyptian Rebirth II</li>
        <li>Egyptian Rebirth II 10 Line</li>
        <li>Fruits Collection – 10 Lines</li>
        <li>Gangster's Gold</li>
        <li>Goddess Of Lotus</li>
        <li>Goddess Of Lotus- 10 Lines</li>
        <li>Magical Amazon</li>
        <li>Penny Fruits Xtreme</li>
        <li>Queen Of Fire</li>
        <li>Spinning Beers</li>
        <li>Story of the Samurai</li>
        <li>Story of the Samurai – 10 Lines</li>
      </ul>

            <h2 style={{color: '#fff',  paddingBottom: '5px', fontWeight: "bold", fontSize: '18px' }}>Spribe</h2>
      <ul>
        <li>Aviator</li>
      </ul>


      <h2 style={{color: '#fff',  paddingBottom: '5px', fontWeight: "bold", fontSize: '18px' }}>Stake Originals</h2>
      <ul>
        <li>Blackjack</li>
        <li>Cases</li>
        <li>Crash</li>
        <li>Darts</li>
        <li>Dice</li>
        <li>Dragon Tower</li>
        <li>Flip</li>
        <li>Hilo</li>
        <li>Keno</li>
        <li>Limbo</li>
        <li>Mines</li>
        <li>Plinko</li>
        <li>Pump</li>
        <li>Rock Paper Scissors</li>
        <li>Slide</li>
        <li>Snakes</li>
      </ul>


      <h2 style={{color: '#fff',  paddingBottom: '5px', fontWeight: "bold", fontSize: '18px' }}>Stake Poker</h2>
      <ul>
        <li>Stake Poker</li>
      </ul>


      <h2 style={{color: '#fff',  paddingBottom: '5px', fontWeight: "bold", fontSize: '18px' }}>Thunderkick</h2>
      <ul>
        <li>12 Bolts of Thunder</li>
        <li>1429 Uncharted Seas</li>
        <li>Beat the Beast: Quetzalcoatl's Trial</li>
        <li>Not Enough Kittens</li>
        <li>Raven's Eye</li>
        <li>Rocket Fellas Inc</li>
        <li>The Rift</li>
        <li>Ways of Athena</li>
      </ul>


      <h2 style={{color: '#fff',  paddingBottom: '5px', fontWeight: "bold", fontSize: '18px' }}>Truelab</h2>
      <ul>
        <li>Aztec Legend</li>
        <li>Crypts of Fortune</li>
        <li>Day and Night</li>
        <li>Moooving Wilds</li>
        <li>Victoria Wild</li>
        <li>Victoria Wild Deluxe</li>
      </ul>

      <h2 style={{color: '#fff', paddingBottom: '5px', fontWeight: "bold", fontSize: '18px' }}>VoltEnt</h2>
      <ul>
        <li>All games</li>
      </ul>


      <h2 style={{color: '#fff', paddingBottom: '5px', fontWeight: "bold", fontSize: '18px' }}>Wazdan</h2>
      <ul>
        <li>All games</li>
      </ul>




    </div>
  );
}






